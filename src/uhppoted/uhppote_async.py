...
class UhppoteAsync:
    ...
    async def get_card_record(self, controller, card_number, timeout=2.5):
        """
        Retrieves the card access record for a card number from the access controller.
            Parameters:
               controller (uint32|tuple)  Controller serial number or tuple with (controller_id,address,protocol)
    ...
    async def get_card_record_by_index(self, controller, card_index, timeout=2.5):
        """
        Retrieves the card access record for a card record from the access controller.
            Parameters:
               controller (uint32|tuple)  Controller serial number or tuple with (controller_id,address,protocol)
    ...
    async def get_time_profile_record(self, controller, profile_id, timeout=2.5):
        """
        Retrieves a time profile from an access conntroller.
    ...
    async def set_time_profile_record(self, controller, profile, timeout=2.5):
        """
    ...
    async def set_door_passcodes_record(self, controller, door, passcodes, timeout=2.5):
        """
        Sets up to four supervisor passcodes for a door. The passcodes override any other access
