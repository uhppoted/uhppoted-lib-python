...
async def get_status_record(u, dest, timeout, args, protocol="udp"):
    """
    Retrieves the controller current state using the 'get_status' API function.
    """
    controller = (args.controller, dest, protocol)

    return await u.get_status_record(controller, timeout=timeout)
...
async def put_card_record(u, dest, timeout, args, protocol="udp"):  # pylint: disable=unused-argument
    ...
    return await u.put_card_record(controller, card, timeout=timeout)
...
async def get_event_record(u, dest, timeout, args, protocol="udp"):
    """
    Retrieves the information for an event from a controller using the 'get_event' API function.
    """
    controller = (args.controller, dest, protocol)
    ...
    return await u.get_event_record(controller, index, timeout=timeout)
...
async def get_time_profile_record(u, dest, timeout, args, protocol="udp"):
    """
    Retrieves a time profile from a controller using the 'get_time_profile_record' API function.
    """
    controller = (args.controller, dest, protocol)
    profile_id = args.profile

    return await u.get_time_profile_record(controller, profile_id, timeout=timeout)
...
async def set_time_profile(u, dest, timeout, args, protocol="udp"):
    ...
    response = await u.set_time_profile(
        controller,
        profile_id,
        start,
        end,
...
async def set_door_passcodes_record(u, dest, timeout, args, protocol="udp"):
    """
    ...
    response = await u.set_door_passcodes_record(controller, door, passcodes, timeout=timeout)
